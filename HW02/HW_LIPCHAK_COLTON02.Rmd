...
$$ 
\log \left( p \left( H \mid AB, \mu \right) \right) \propto log[\mu]\times H + log[1-\mu] \times (AB - H)
$$
...
{
  # assign log_lik
  log_lik <- log(prob)*events + log(1-prob)* (trials - events)
  # return log_lik
  return(log_lik)
}
```
...
{
  # your code here
  log_lik <- lchoose(trials, events) + log(prob)*events + log(1-prob)* (trials - events) 
  return(log_lik)
}
```
...
?  
The MLE corresponds to the same spot. 
...
### your code here
my_probability <- exp(log_binom_pmf(189,602,0.3))
my_probability
```
...
### your code here
r_probability <- dbinom(189,602,0.3,FALSE)
r_probability
```
...
tibble::tibble(
  ...
  mutate(log_lik = dbinom(player_hits,player_atbats,mu,TRUE))%>% 
  ggplot(mapping = aes(x = mu, y = log_lik)) +
  geom_line(mapping = aes(x = mu, y = log_lik),
            size = 1.2) +
  geom_vline(xintercept = player_mle,
...
coin_probs <- dbinom(0:5,11,0.25,FALSE)
...
? 
The first element corresponds to the probability of getting 1 or less heads
...
```{r, solution_02g}
### your code here
prob175to195 <- pbinom(195,602,0.3) - pbinom(175,602,0.3)
prob175to195

prob165to205 <- pbinom(205,602,0.3) - pbinom(165,602,0.3)
prob165to205
```
...
$$ 
\log \left( p \left(\mu \mid a, b \right) \right) \propto  log(\mu^{a-1}(1-\mu)^{b-1}) = (a-1)\times log(\mu)+(b-1)\times log(1-\mu)
$$
...
$$
unnormalized \space log\space likelihood = 
log[\mu]\times H + log[1-\mu] \times (AB - H)
$$
$$
unnormalized \space log\space prior = log[\mu]\times (a-1)+log[1-\mu] \times (b-1) 
$$
$$
log[\mu]\times H + log[1-\mu] \times (AB - H) = log[\mu]\times (a-1)+log[1-\mu] \times (b-1) 
$$
### 3c)
...
```{r, solution_03d}
### your code here
quantile5th <- qbeta(0.05,a_prior,b_prior)
quantile5th

quantile95th <- qbeta(0.95,a_prior,b_prior)
quantile95th
```

?
The MLE based on the first 30 at bats (0.2) is very close to the 0.05 quantile. The MLE based on all 602 at bats in the season is 0.314 so it is very close to the 0.95 quantile, but not close to the 0.05 quantile. 
...
a_post_start <- a_prior * start_mle
b_post_start <- b_prior * start_mle

a_post_player <- a_prior * player_mle
b_post_player <- b_prior * player_mle
```
...
The posterior means are the same. They appear to be in between both of their corresponding MLEs (0.2 - 0.314). 
...
quant5_for_30 <- qbeta(0.05,a_post_start,b_post_start)
quant5_for_30
quant95_for_30 <-qbeta(0.95,a_post_start,b_post_start)
quant95_for_30

quant5_for_602 <- qbeta(0.05,a_post_player,b_post_player)
quant5_for_602
quant95_for_602 <-qbeta(0.95,a_post_player,b_post_player)
quant95_for_602
```
The MLEs (0.2 - 0.314) are contained within the intervals.
...
prior_hitprob <- pbeta(0.3,quant5_for_30,quant95_for_30,lower.tail = FALSE)
prior_hitprob

post_hitprob <- pbeta(0.3,quant5_for_602,quant95_for_602,lower.tail = FALSE)
post_hitprob
```
There is only a 38% posterior probability that the player's hit probability is greater than 0.3, so I would not feel comfortable saying that the player is a good hitter. 
## Problem 4
...
$$
p \left(\sigma \mid \mathbf{x}, \mu\right) \propto \prod_{n=1}^{N} \left(\mathrm{normal}\left( x_n \mid \mu, \sigma \right) \right) \times  \mathrm{Exp}\left( \sigma \mid \lambda \right) = \prod_{n=1}^N(\frac{1}{\sqrt{2\pi\sigma^2}}\exp(-\frac{(x_n-\mu)^2}{2\sigma^2})\lambda\exp(-\lambda\sigma))
$$

$$
Take \space  the \space log: \sum_{n=1}^Nlog[(\frac{1}{\sqrt{2\pi\sigma^2}}\exp(-\frac{(x_n-\mu)^2}{2\sigma^2})) \lambda\exp(-\lambda\sigma)]
$$
$$
Expand: \sum_{n=1}^Nlog[(-\frac{1}{2}log[2\pi]-\frac{1}{2}log[\sigma^2]-\frac{1}{2\sigma^2}(x_n-\mu)^2) \lambda\exp(-\lambda\sigma)]
$$
$$
drop \space non \space '\mu' \space terms: \sum_{n=1}^N-\frac{1}{2\sigma^2}(x_n-\mu)^2\lambda\exp(-\lambda\sigma))
$$
...
$$
v\times \sum_{n=1}^{N}\lambda exp(-\lambda\sigma))
$$
It is referred to as sufficient because it covers all of the information included from the sample. The expression no long depends on the individual observations after making use of v. 
...
{
  # your code here
  log_post <- v * N * lambda *exp(-lambda*sigma)
  return(log_post)
}
```
...
tibble::tibble(
  ...
  geom_line(mapping = aes(x = sigma, y = log_post),
            size = 1.2) 
```

?  
It has one mode and it looks like half of a parabola.
