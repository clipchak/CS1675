...
```{r, solution_01a}
df_01 %>% 
  ggplot(mapping = aes(x = df_01$obs_id, y = df_01$x)) +
  geom_line(mapping = aes(x =df_01$obs_id , y =  df_01$x),
            size = 0.5) + 
  geom_point(mapping = aes(x =df_01$obs_id , y =  df_01$x),
             size = 3 )
```
There are a few values appearing to stand out. Early on, one value of x = ~17 is an outlier. Late in the obs_id measurement, there are multiple values that appear to be outside of the main range. 
...
```{r, solution_01b}
df_01 %>% 
  ggplot(mapping = aes(x = df_01$x)) +
  geom_histogram(bins = 25)
```

We get a normal distribution with most of the values concentrated around x = 0 and spread out mostly over x = -10 to x = 10.  
...
quantile(df_01$x,probs = c(0.05, 0.95))

### calculate the empirical standard deviation of x
sd(df_01$x)
```

These summary statistics confirm my conclusions based on the run chart and histogram. The median of the data is 0.69 meaning that most of the values are around there. Only 5% of the points lie below x = -7.8 and 95% of the data is less than x = 9.23.
...
```{r, solution_01d}
df_01 %>% 
  mutate(rolling_avg = cummean(df_01$x)) %>% 
  ggplot(mapping = aes(x = df_01$obs_id, y = rolling_avg)) +
  geom_line(mapping = aes(x =df_01$obs_id , y =  rolling_avg),
            size = 0.5)
```
The first portion of the rolling average is the most variable. It appears to steady out around a value of .75.
...
```{r, solution_01e}
avg_after_5 <- cummean(df_01$x)[5]
avg_after_5

avg_after_45 <- cummean(df_01$x)[45]
avg_after_45
```
...
So that we can allow all values from negative infinity to positive infinity. 
...
$$
p\left(\mu,\varphi \mid \mathbf{x} \right) \propto \prod_{n=1}^{N} p(x_n|\mu,g^{-1}(\varphi))p(\mu)p(\sigma)
$$

$$
p\left(\mu,\varphi \mid \mathbf{x} \right) \propto \prod_{n=1}^{N} p(x_n|\mu,g^{-1}(\varphi))p(\mu|\mu_0,\tau_0)p(g^{-1}(\varphi)|\lambda)
$$
...
my_logpost <- function(unknowns, my_info)
{
  ...
  log_deriv_adjust <- log(exp(lik_varphi))
...
### bounds on mu
mu_lwr <- qnorm(p = 0.01, mean = info_inform_N05$mu_0, sd = info_inform_N05$tau_0)
mu_upr <- qnorm(p = 0.99, mean = info_inform_N05$mu_0, sd = info_inform_N05$tau_0)

### bounds on varphi
varphi_lwr <- log(qexp(p = 0.01, rate = info_inform_N05$sigma_rate))
varphi_upr <- log(qexp(p = 0.99, rate = info_inform_N05$sigma_rate))
...
{
  my_func(unknowns = c(mu_val,varphi_val) , my_info)
}
```
...
Values from 0.5 to -3.0... for $\varphi$ are completely ruled out. The most probable values for $\mu$ appear to be between -1 and 0. 
...
As the sample size increases, the posterior narrows in on its value. We come closer to the true value. The visualization narrows in its height and width. The uncertainty decreases as the sample size increases. 
...
laplace_res_inform_N05 <- my_laplace(c(0,0),my_logpost,info_inform_N05 )

laplace_res_inform_N45 <- my_laplace( c(-1,-1),my_logpost,info_inform_N45 )

laplace_res_inform_N405 <- my_laplace(c(2,2),my_logpost,info_inform_N405  )
...
post_N05_mean_mu <- mean(post_samples_inform_N05$mu)
post_N05_mean_sigma <- mean(post_samples_inform_N05$sigma)

post_N45_mean_mu <- mean(post_samples_inform_N45$mu)
post_N45_mean_sigma <- mean(post_samples_inform_N45$sigma)

post_N405_mean_mu <- mean(post_samples_inform_N405$mu)
post_N405_mean_sigma <- mean(post_samples_inform_N405$sigma)


p_N05_sigma <- post_samples_inform_N05 %>% 
  ggplot(mapping = aes(x = post_samples_inform_N05$sigma)) +
  geom_histogram(bins = 25) 

p_N05_mu <- post_samples_inform_N05 %>% 
  ggplot(mapping = aes(x = post_samples_inform_N05$mu)) +
  geom_histogram(bins = 25) 

ggarrange(p_N05_sigma, p_N05_mu, ncol =2, nrow = 1, labels = c("N05 sigma", "N05 mu"))

p_N45_sigma <- post_samples_inform_N45 %>% 
  ggplot(mapping = aes(x = post_samples_inform_N45$sigma)) +
  geom_histogram(bins = 25) 

p_N45_mu <- post_samples_inform_N45 %>% 
  ggplot(mapping = aes(x = post_samples_inform_N45$mu)) +
  geom_histogram(bins = 25) 

ggarrange(p_N45_sigma, p_N45_mu, ncol =2, nrow = 1, labels = c("N45 sigma", "N45 mu"))

p_N405_sigma <- post_samples_inform_N405 %>% 
  ggplot(mapping = aes(x = post_samples_inform_N405$sigma)) +
  geom_histogram(bins = 25) 

p_N405_mu <- post_samples_inform_N405 %>% 
  ggplot(mapping = aes(x = post_samples_inform_N405$mu)) +
  geom_histogram(bins = 25) 

ggarrange(p_N405_sigma, p_N405_mu, ncol =2, nrow = 1, labels = c("N405 sigma", "N405 mu"))
...
prob_N05_positive <- pnorm(0,mean = post_N05_mean_mu, sd = post_N05_mean_sigma, lower.tail = FALSE )
prob_N05_positive

prob_N45_positive <- pnorm(0,mean = post_N45_mean_mu, sd = post_N45_mean_sigma, lower.tail = FALSE )
prob_N45_positive

prob_N405_positive <- pnorm(0,mean = post_N405_mean_mu, sd = post_N405_mean_sigma, lower.tail = FALSE )
prob_N405_positive

quant05_N05 <- qnorm(p = .05, mean = post_N05_mean_mu, sd = post_N05_mean_sigma)
quant05_N05
quant95_N05 <- qnorm(p = .95, mean = post_N05_mean_mu, sd = post_N05_mean_sigma)
quant95_N05

quant05_N45 <- qnorm(p = .05, mean = post_N45_mean_mu, sd = post_N45_mean_sigma)
quant05_N45
quant95_N45 <- qnorm(p = .95, mean = post_N45_mean_mu, sd = post_N45_mean_sigma)
quant95_N45

quant05_N405 <- qnorm(p = .05, mean = post_N405_mean_mu, sd = post_N405_mean_sigma)
quant05_N405
quant95_N405 <- qnorm(p = .95, mean = post_N405_mean_mu, sd = post_N405_mean_sigma)
quant95_N405
```
...
#### SOLUTION
Based on its graph, df_02_low's true functional form looks to be quadratic.

```{r, solution_4a}
df_02_low %>% 
  ggplot(mapping = aes(x = df_02_low$x )) +
  geom_point(mapping = aes(y = df_02_low$y),
             size = 3)
df_02_high %>% 
  ggplot(mapping = aes(x = df_02_high$x )) +
  geom_point(mapping = aes(y = df_02_high$y),
             size = 3)
```
...
mod_low_linear <- lm(df_02_low$y ~ df_02_low$x, data = df_02_low)

mod_low_quad <- lm(df_02_low$y ~ df_02_low$x + I(df_02_low$x^2), data = df_02_low)

mod_low_8th <- lm(df_02_low$y ~ df_02_low$x + I(df_02_low$x^2) + I(df_02_low$x^8), data = df_02_low)
```

The high noise case models are given in the code chunk below.  

```{r, solution_04b_b, eval=TRUE}
mod_high_linear <- lm(df_02_high$y ~ df_02_high$x , data = df_02_high)

mod_high_quad <- lm(df_02_high$y ~ df_02_high$x + I(df_02_high$x^2), data = df_02_high)

mod_high_8th <- lm(df_02_high$y ~ df_02_high$x + I(df_02_high$x^2) + I(df_02_high$x^8), data = df_02_high)
```
...
For the low noise case, the quadratic and the 8th had the highest R-squared. For the high noise case, the quadratic had the highest R-squared, but it was also pretty close to the 8th's value for R-squared. 
...
mod_low_quad_fold_01 <- lm(low_noise_train_fold_01$y ~ low_noise_train_fold_01$x + I(low_noise_train_fold_01$x^2), data = low_noise_train_fold_01)

mod_low_quad_fold_02 <- lm(low_noise_train_fold_02$y ~ low_noise_train_fold_02$x + I(low_noise_train_fold_02$x^2), data = low_noise_train_fold_02)

mod_low_quad_fold_03 <- lm(low_noise_train_fold_03$y ~ low_noise_train_fold_03$x + I(low_noise_train_fold_03$x^2), data = low_noise_train_fold_03)

mod_low_quad_fold_04 <- lm(low_noise_train_fold_04$y ~ low_noise_train_fold_04$x + I(low_noise_train_fold_04$x^2), data = low_noise_train_fold_04)

mod_low_quad_fold_05 <- lm(low_noise_train_fold_05$y ~ low_noise_train_fold_05$x + I(low_noise_train_fold_05$x^2), data = low_noise_train_fold_05)
```

The 8th order polynomial fits should be specified below.  

```{r, solution_04f_b, eval=TRUE}
mod_low_8th_fold_01 <- lm(low_noise_train_fold_01$y ~ low_noise_train_fold_01$x + I(low_noise_train_fold_01$x^2) + I(low_noise_train_fold_01$x^8), data = low_noise_train_fold_01)

mod_low_8th_fold_02 <- lm(low_noise_train_fold_02$y ~ low_noise_train_fold_02$x + I(low_noise_train_fold_02$x^2) + I(low_noise_train_fold_02$x^8), data = low_noise_train_fold_02)

mod_low_8th_fold_03 <- lm(low_noise_train_fold_03$y ~ low_noise_train_fold_03$x + I(low_noise_train_fold_03$x^2) + I(low_noise_train_fold_03$x^8), data = low_noise_train_fold_03)

mod_low_8th_fold_04 <- lm(low_noise_train_fold_04$y ~ low_noise_train_fold_04$x + I(low_noise_train_fold_04$x^2) + I(low_noise_train_fold_04$x^8), data = low_noise_train_fold_04)

mod_low_8th_fold_05 <- lm(low_noise_train_fold_05$y ~ low_noise_train_fold_05$x + I(low_noise_train_fold_05$x^2) + I(low_noise_train_fold_05$x^8), data = low_noise_train_fold_05)
```
...
cv_low_quad_fold_train_rmse_avg <- mean(cv_low_quad_fold_train_rmse)
cv_low_quad_fold_train_rmse_avg

cv_low_8th_fold_train_rmse_avg <- mean(cv_low_8th_fold_train_rmse)
cv_low_8th_fold_train_rmse_avg

```
 
They are both very similar, but the 8th order polynomial has the lower RMSE.
...
```{r, solution_04h}
mod_low_quad_fold_01_test <- lm(low_noise_test_fold_01$y ~ low_noise_test_fold_01$x + I(low_noise_test_fold_01$x^2), data = low_noise_test_fold_01)

mod_low_quad_fold_02_test <- lm(low_noise_test_fold_02$y ~ low_noise_test_fold_02$x + I(low_noise_test_fold_02$x^2), data = low_noise_test_fold_02)

mod_low_quad_fold_03_test <- lm(low_noise_test_fold_03$y ~ low_noise_test_fold_03$x + I(low_noise_test_fold_03$x^2), data = low_noise_test_fold_03)

mod_low_quad_fold_04_test <- lm(low_noise_test_fold_04$y ~ low_noise_test_fold_04$x + I(low_noise_test_fold_04$x^2), data = low_noise_test_fold_04)

mod_low_quad_fold_05_test <- lm(low_noise_test_fold_05$y ~ low_noise_test_fold_05$x + I(low_noise_test_fold_05$x^2), data = low_noise_test_fold_05)

mod_low_8th_fold_01_test <- lm(low_noise_test_fold_01$y ~ low_noise_test_fold_01$x + I(low_noise_test_fold_01$x^2) + I(low_noise_test_fold_01$x^8), data = low_noise_test_fold_01)

mod_low_8th_fold_02_test <- lm(low_noise_test_fold_02$y ~ low_noise_test_fold_02$x + I(low_noise_test_fold_02$x^2) + I(low_noise_test_fold_02$x^8), data = low_noise_test_fold_02)

mod_low_8th_fold_03_test <- lm(low_noise_test_fold_03$y ~ low_noise_test_fold_03$x + I(low_noise_test_fold_03$x^2) + I(low_noise_test_fold_03$x^8), data = low_noise_test_fold_03)

mod_low_8th_fold_04_test <- lm(low_noise_test_fold_04$y ~ low_noise_test_fold_04$x + I(low_noise_test_fold_04$x^2) + I(low_noise_test_fold_04$x^8), data = low_noise_test_fold_04)

mod_low_8th_fold_05_test <- lm(low_noise_test_fold_05$y ~ low_noise_test_fold_05$x + I(low_noise_test_fold_05$x^2) + I(low_noise_test_fold_05$x^8), data = low_noise_test_fold_05)



cv_low_quad_fold_test_rmse <- c(modelr::rmse( mod_low_quad_fold_01_test, low_noise_test_fold_01),
                                 modelr::rmse( mod_low_quad_fold_02_test, low_noise_test_fold_02),
                                 modelr::rmse( mod_low_quad_fold_03_test, low_noise_test_fold_03),
                                 modelr::rmse( mod_low_quad_fold_04_test, low_noise_test_fold_04),
                                 modelr::rmse( mod_low_quad_fold_05_test, low_noise_test_fold_05))
cv_low_8th_fold_test_rmse <- c(modelr::rmse( mod_low_8th_fold_01_test, low_noise_test_fold_01),
                                 modelr::rmse( mod_low_8th_fold_02_test, low_noise_test_fold_02),
                                 modelr::rmse( mod_low_8th_fold_03_test, low_noise_test_fold_03),
                                 modelr::rmse( mod_low_8th_fold_04_test, low_noise_test_fold_04),
                                 modelr::rmse( mod_low_8th_fold_05_test, low_noise_test_fold_05))

cv_low_quad_fold_test_rmse_avg <- mean(cv_low_quad_fold_test_rmse)
cv_low_quad_fold_test_rmse_avg

cv_low_8th_fold_test_rmse_avg <- mean(cv_low_8th_fold_test_rmse)
cv_low_8th_fold_test_rmse_avg

```

The 8th order polynomial has the lowest RMSE.