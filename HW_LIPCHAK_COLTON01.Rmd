...
We discretized Sepal.Length into bins with certain intervals and are now counting the number of occurences in these intervals. These new values along the x-axis come from calling 'cut()' on Sepal.Length using the hist_breaks that we defined. 
...
The resulting counts are the same as the result from the 'count()' function.
...
The manual histogram is very similar to the ggplot histogram. There are two points that were right on the mark, three points slightly below the red line, and one point in the middle was off the mark pretty noticeably. But overall, the manual histogram performed well compared to the native. 
...
diamonds %>% 
  ...
  geom_boxplot(aes(fill = color), outlier.colour = "blue") +
  theme(legend.position = "top")
```

On average, Color J is the most expensive. Colors D and E are the least expensive on average. Color E has the longest stretch of outliers while Color J has the shortest stretch of outliers. The boxes are mostly toward the bottom of the graph.
...
diamonds %>% 
  ggplot(mapping = aes(x = color, y = price)) + 
  geom_boxplot(aes(fill = carat > median(carat)), outlier.colour = "blue") +
  facet_grid(clarity ~ cut) +
  theme(legend.position = "top")
```
...
diamonds %>% 
  ...
  ggplot(mapping = aes(x = color, y = price)) + 
  geom_boxplot(aes(fill = carat > median(diamonds$carat)), outlier.colour = "blue") +
  facet_grid(clarity ~ cut) +
  theme(legend.position = "top")
```
...
In the figure from 5f), the boxes for the true case are mostly concentrated toward the higher prices while it was the opposite in the figure from 5b). There are much less outliers in 5f) as compared to 5b). Color J has a very large range and now no outliers. Color D has a very small range and no outliers.
...
$$ 
\frac{\partial f}{\partial x_1} = bax_{1}^{b-1}
$$
...
$$ 
\frac{\partial f}{\partial x_2} = dcx_{2}^{d-1}
$$
...
$$ 
\frac{\partial^{2} f}{\partial x_{1}^{2 }} = (b-1)bax_{1}^{b-2}
$$
...
$$ 
\frac{\partial^{2} f}{\partial x_{1} \partial x_{2}} = (b-1)bax_{1}^{b-2} + (d-1)dcx_{2}^{d-2}
$$
...
$$ 
\mathbf{x} \cdot \mathbf{x} = x_{1}x_{1} + x_{2}x_{2} + ... + x_{n}x_{n}
$$
...
$$
\text{Inner Product: }\\
\mathbf{x}^T\mathbf{x} = \left(1 \times N\right) \cdot \left(N \times 1\right) = \left(1 \times 1\right)
$$
$$
\text{Outer Product: }\\
\mathbf{xx}^T  = \left(N \times 1\right) \cdot \left(N \times 1\right) = \left(N \times N\right)
$$
